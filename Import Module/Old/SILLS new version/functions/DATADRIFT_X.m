% DATADRIFT
function [A, UNK,STD, SRM] = DATADRIFT_X(A, UNK,STD, SRM)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
for d = 1:A.STD_num
    for c = 1:A.ISOTOPE_num
        ...
        % replaced numeric Id of SRM with comparing strings of SRM
        % names and selected SRM to find Id
        % old version is numeric, new version goes by strings of names
        %  temp2 = strcmp(SRM(STD(d).SRM).rowheaders,char(temp)); %compare the element list with the appropriate SRM list
        temp2 = strcmp(SRM(contains({SRM.name},STD(d).SRM)).rowheaders,char(temp)); %compare the element list with the appropriate SRM list
        temp3 = find(temp2==1);
        if ~isempty(temp3);
            % replaced numeric Id of SRM with comparing strings of SRM
            % names and selected SRM to find Id
            ...
            STD(d).SRM_concs(c) = SRM(contains({SRM.name},STD(d).SRM)).data(temp3);
        else
            STD(d).SRM_concs(c) = 0;
        end
    end
