...
norm_data=false;
if norm_data==true
    A0=normalize(A0,'range');
    B0=normalize(B0,'range');
    C0=normalize(C0,'range');
    D0=normalize(D0,'range');
end
...
if isfield(options,'ColorData') && isfield(options.ColorData,'Value') && options.ColorData.Value && isfield(options.ColorData,'colormap') && not(isempty(options.ColorData.colormap))
    ...
    s=  scatter3(ax2plot,X1(:),Y1(:),Z1(:),symbsize,C1(:),symb,'filled','MarkerFaceAlpha',mfa,'MarkerEdgeAlpha',mea,'MarkerEdgeColor',mec,'LineWidth',mlw);
    cbar= colorbar;
    ...
    s=  scatter3(ax2plot,X1(:),Y1(:),Z1(:),symbsize,symb,'filled','MarkerFaceAlpha',mfa,'MarkerEdgeAlpha',mea,'MarkerEdgeColor',mec,'MarkerFaceColor',fil,'LineWidth',mlw);
    if any(not(Z1==0))
        view(3)
    else
        view(2)
