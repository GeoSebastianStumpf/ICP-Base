...
options_definition.type.options={'none';
    ...
    'clinopyroxene_xMg_Endmember_short';
...
if options.XYplot.Value==true
    ...
    elseif not(isempty(options)) && isfield(options,'type') && strcmp(options.type.Value,'clinopyroxene_xMg_NaO2_Cr2O3_Al2O3')
        ...
        fig_resize=[1 1 1.5 1.5 ];
    elseif not(isempty(options)) && isfield(options,'type') && strcmp(options.type.Value,'clinopyroxene_xMg_Endmember')
        ax2plot=tiledlayout('flow','TileSpacing','tight');
        x_label_str={'X_{Mg}'  ,'X_{Mg}'  ,'X_{Mg}'   ,'X_{Mg}'  ,'X_{Mg}' ,'X_{Mg}','X_{Mg}','X_{Mg}' ,'X_{Mg}','X_{Mg}','X_{Mg}' ,'X_{Mg}'  ,'X_{Mg}' ,'X_{Mg}' ,'X_{Mg}','X_{Mg}'};
        y_label_str={'X_{CrAl}','X_{AlAl}','X_{Fe3Al}','X_{TiAl}','X_{tss}','X_{di}','X_{hd}','X_{jhn}','X_{en}','X_{fs}','X_{rho}','X_{Kcpx}','X_{aeg}','X_{kos}','X_{jd}','X_{NaTi}'};

        fig_resize=[1 1 2.75 1];
    elseif not(isempty(options)) && isfield(options,'type') && strcmp(options.type.Value,'clinopyroxene_xMg_Endmember_short')
...
try
    for n=1:numel(ax2plot)
% ---- CONTEXT MENU FOR COPYING AXIS ----
                    cm = uicontextmenu(hfig);
                    ax2plot(n).UIContextMenu = cm;
                    uimenu(cm, ...
                        'Label','Open axis in new window', ...
                    'Callback', @(~,~)copyAxisToNewFigure(ax2plot(n)));
                    uimenu(cm, ...
                        'Label','Send to EasyPlotModule', ...
                    'Callback', @(~,~)AxisToEasyPlot(ax2plot(n)));
...
newAx.XMinorGrid='on';
newAx.YMinorGrid='on';
l=legend(newAx,unique({newAx.Children.Tag}));
l.Interpreter='none';
...
newAx.XLabel.String=ax2plot.XLabel.String{:};

newAx.YLabel.String=ax2plot.YLabel.String{:};

newAx.FontSize=18;

newAx.XScale=ax2plot.XScale;
newAx.YScale=ax2plot.YScale;
