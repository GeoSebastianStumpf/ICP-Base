...
elseif not(isempty(options)) && isfield(options,'type') && strcmp(options.type.Value,'tern_carbonate_Ca_Mg_Fe')
 ...
 elseif not(isempty(options)) && isfield(options,'type') && strcmp(options.type.Value,'tern_ep_cz_pm')

 %% epidote-clinozoesite-pimonitite ternary

    str_ep1={'Al^{3+}';'Mn^{3+}';'Fe^{3+}'}; %endmember text labels
    str_ep2={'Clinozoisite';'Piemontite';'Epidote'}; %endmember text labels
    ...
    text(ax2plot,pos_ep1(:,1),pos_ep1(:,2),str_ep1,'FontSize',options.FontSize.Value*1.1667) %endmembers
    text(ax2plot,pos_ep2(:,1),pos_ep2(:,2),str_ep2,'FontSize',options.FontSize.Value*1.1667,'HorizontalAlignment','center','VerticalAlignment','middle') %endmembers
